...
export function LandingPageCalculator() {
  ...
  const { 
    ...
    // Constants
    const HOURS_PER_WEEK = hoursPerWeek[0];
    const MONTHLY_HOURS = HOURS_PER_WEEK * (52 / 12);
    
    // STEP 1: Client -> Candidate
    const clientRate = hourlyRate[0];
    ...
    // Formula: CandidateRate / 2.0 = GrossHourly
    const candidateRate = clientRate * (1 - marginPct); // 85% of client rate
    ...
    // 4a. Calculate pensionable wage (subtract franchise)
    // Franchise is a fixed annual amount divided by 12 for monthly calculation
    const annualFranchise = config.annualFranchise; // Fixed annual franchise amount
    const monthlyFranchise = annualFranchise / 12;
    const pensionableWageVal = grossMonthly - monthlyFranchise;
    
    // 4b. Calculate pension contributions
    const employerPensionVal = pensionableWageVal * config.employerPensionRate; // 15.9% employer contribution (company cost, not deducted from gross)
    const employeePensionVal = pensionableWageVal * config.employeePensionRate; // 7.5% employee contribution (deducted from gross, pre-tax)
    
    // STEP 5: Other Fiscal Reservations (Pre-tax)
    // AZV 0.3%, PAWW 0.1% - only if applicable for this CAO
    ...
    // Convert monthly to annual for tax calculations
    ...
    const rate1 = config.taxRate1; // 36.97% (includes social premiums)
    const rate2 = config.taxRate2; // 49.50%
    ...
    const akMax = config.arbeidsKortingMax; // Max annual work credit
    ...
    const ahkMax = config.algemeenHeffingsKortingMax; // Max annual general credit
    ...
    // The 36.97% rate consists of approximately:
    // - 18.75% income tax
    // - 18.22% social premiums (AOW, Anw, WLZ)
    const socialPremiumRate = config.socialPremiumRate;
    const incomeTaxRate = rate1 - socialPremiumRate; // ~18.75%
    ...
    const holidayHours = MONTHLY_HOURS * config.holidayHoursRate; // 10.92% of work hours = hours reserved for vacation
    const netHourlyRate = netMonthly / MONTHLY_HOURS; // Calculate net hourly rate
    const holidayDaysEquivalent = holidayHours * netHourlyRate; // Monetary value of vacation hours at net rate
    const holidayAllowance = grossMonthly * config.holidayAllowanceRate; // 8% - vakantiegeld (always applicable)
    const yearEndBonus = config.hasYearEndBonus ? grossMonthly * config.yearEndBonusRate : 0; // 4.5% - eindejaarsuitkering (CAO specific)
    const ikbContribution = config.hasIKB ? grossMonthly * config.ikbRate : 0; // 1.8% - IKB bijdrage (CAO specific)
    ...
    return {
      ...
      taxesTotal: totalMonthlyDeductions, // For compatibility
      netTotal: netMonthly,
  ...
  return (
    ...
    <div className="min-h-screen bg-slate-50 font-sans text-slate-900 pb-20">
      ...
      <button
        ...
        className="fixed bottom-8 right-8 bg-slate-800 hover:bg-slate-700 text-white p-4 rounded-full shadow-lg transition-all z-50 flex items-center gap-2"
      ...
      {showConfig && (
        <div className="fixed inset-0 bg-black/50 z-40 flex items-start justify-end p-4 overflow-y-auto" onClick={() => setShowConfig(false)}>
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-2xl my-8" onClick={(e) => e.stopPropagation()}>
            <div className="sticky top-0 bg-slate-800 text-white p-6 rounded-t-xl z-10">
              ...
              <p className="text-slate-300 text-sm mt-2">Pas percentages, CAO-instellingen en berekeningsparameters aan</p>
            </div>
            ...
            <div className="p-6 space-y-6 max-h-[calc(100vh-200px)] overflow-y-auto">
              ...
              <div className="space-y-4 bg-amber-50 p-4 rounded-lg border border-amber-200">
                ...
                <div className="space-y-3">
                  <label className="flex items-center justify-between p-3 bg-white rounded border border-amber-200">
                    <span className="text-sm font-medium text-slate-700">AZV (Aanvullende Ziektekostenverzekering)</span>
                    <input
                      type="checkbox"
                  ...
                  <label className="flex items-center justify-between p-3 bg-white rounded border border-amber-200">
                    <span className="text-sm font-medium text-slate-700">PAWW (Premie Aanvullende Werknemersverzekeringen)</span>
                    <input
                      type="checkbox"
                  ...
                  <label className="flex items-center justify-between p-3 bg-white rounded border border-amber-200">
                    <span className="text-sm font-medium text-slate-700">Eindejaarsuitkering (13e maand)</span>
                    <input
                      type="checkbox"
                  ...
                  <label className="flex items-center justify-between p-3 bg-white rounded border border-amber-200">
                    <span className="text-sm font-medium text-slate-700">IKB (Individueel Keuze Budget)</span>
                    <input
                      type="checkbox"
              ...
              <div className="space-y-4">
                <h3 className="font-bold text-lg text-slate-900 border-b pb-2">Bedrijfsmarge</h3>
                <ConfigInput
                  label="Totale marge (%)"
              ...
              {/* Conversion */}
              <div className="space-y-4">
                <h3 className="font-bold text-lg text-slate-900 border-b pb-2">Conversie</h3>
                <ConfigInput
              ...
              {/* Pension */}
              <div className="space-y-4">
                <h3 className="font-bold text-lg text-slate-900 border-b pb-2">Pensioen (StiPP)</h3>
                <ConfigInput
              ...
              {/* Reservations */}
              <div className="space-y-4">
                <h3 className="font-bold text-lg text-slate-900 border-b pb-2">Overige Reserveringen</h3>
                <ConfigInput
                  label="AZV (%)"
              ...
              {/* Tax Brackets */}
              <div className="space-y-4">
                <h3 className="font-bold text-lg text-slate-900 border-b pb-2">Belastingschijven 2026</h3>
                <ConfigInput
              ...
              {/* Tax Credits */}
              <div className="space-y-4">
                <h3 className="font-bold text-lg text-slate-900 border-b pb-2">Heffingskortingen</h3>
                <ConfigInput
              ...
              {/* Benefits */}
              <div className="space-y-4">
                <h3 className="font-bold text-lg text-slate-900 border-b pb-2">Extra Uitkeringen</h3>
                <ConfigInput
                  label="Vakantiedagen (%)"
      ...
      {/* SECTION 1: Intro */}
      <section className="bg-white border-b border-slate-100 pt-16 pb-12 px-4">
        ...
        <div className="max-w-5xl mx-auto grid md:grid-cols-3 gap-6 mt-16 text-left">
          ...
          <Card className="border-slate-100 shadow-sm hover:shadow-md transition-shadow bg-blue-50/50 border-blue-100">
            <CardHeader>
      ...
      <section className="py-16 px-4 max-w-5xl mx-auto space-y-12">
        <div className="text-center space-y-2">
          <h2 className="text-3xl font-bold text-slate-900">Wat krijg jij hier uiteindelijk voor?</h2>
          <p className="text-slate-500">Pas het uurtarief aan en zie direct wat je netto ontvangt én wat er voor je wordt opgebouwd.</p>
        </div>
                  ...
                  <div className="space-y-4">
                    <div className="flex justify-between items-end">
                      ...
                      <div className="flex items-baseline gap-1">
                        ...
                        <span className="text-slate-400 font-medium text-lg whitespace-nowrap">/ uur</span>
                      </div>
                    </div>
                    <Slider
                      value={hourlyRate}
                  ...
                  <div className="space-y-4">
                    <div className="flex justify-between items-end">
                      ...
                      <div className="flex items-baseline gap-1">
                        ...
                        <span className="text-slate-400 font-medium text-lg whitespace-nowrap">uren</span>
                      </div>
                    </div>
                    <Slider
                      value={hoursPerWeek}
        ...
        <div className="grid md:grid-cols-4 gap-4 items-stretch">
            {/* Step 1: Client Pays */}
            <div className="bg-white p-6 rounded-xl border border-slate-200 flex flex-col gap-2 relative group">
                <div className="text-xs font-bold uppercase tracking-wider text-slate-400">Wat de klant betaalt</div>
                <div className="flex flex-col">
                     <span className="text-2xl font-bold text-slate-900">{formatCurrency(clientTotal)}</span>
                     <span className="text-sm font-medium text-slate-400">{formatHourly(clientTotal)} /uur</span>
                </div>
                <div className="text-sm text-slate-500 mt-1">Het volledige maandbedrag</div>
                <ChevronRight className="hidden md:block absolute -right-6 top-1/2 -translate-y-1/2 text-slate-300 w-8 h-8 z-10" />
            </div>

            {/* Step 2: Company Share */}
            <div className="bg-slate-50 p-6 rounded-xl border border-slate-200 flex flex-col gap-2 relative">
                <div className="text-xs font-bold uppercase tracking-wider text-slate-400">Bedrijfskosten & marge</div>
                <div className="flex flex-col mb-3">
                     <div className="flex items-baseline gap-2 mb-1">
                          <span className="text-xs text-slate-500">Winst (5%)</span>
                          <span className="text-2xl font-bold text-slate-900">{formatCurrency(marginBreakdown.profit)}</span>
                     </div>
                     <div className="flex items-baseline gap-2">
                          <span className="text-[10px] text-slate-400">Kosten (10%)</span>
                          <span className="text-sm font-medium text-slate-600">{formatCurrency(marginBreakdown.admin)}</span>
                     </div>
                </div>
                <div className="text-xs text-slate-500 mt-auto">Totaal: {formatCurrency(companyTotal)}</div>
                 <div className="hidden md:block absolute -right-4 top-1/2 -translate-y-1/2 border-t-2 border-slate-200 w-8 z-0"></div>
            </div>
            ...
            <div className="bg-blue-50 p-6 rounded-xl border border-blue-100 flex flex-col gap-2 relative col-span-2">
                ...
                <div className="flex items-end justify-between mb-2 gap-4">
                     ...
                     <div className="flex flex-col items-end">
                          <span className="text-xs text-slate-500 uppercase tracking-wide mb-1">Jouw totale opbrengst</span>
                ...
                <div className="grid grid-cols-3 gap-3 mt-auto pt-4 border-t border-blue-200/50">
                    <div className="bg-white/50 rounded-lg p-3">
                        <div className="text-base font-bold text-slate-900">{formatCurrency(netTotal)}</div>
                        <div className="text-[10px] text-slate-500 font-medium uppercase tracking-wide">Netto loon</div>
                        <div className="text-[10px] text-slate-400">{formatHourly(netTotal)}/u</div>
                    </div>
                    ...
                    <div className="bg-purple-50/80 rounded-lg p-3">
                        <div className="text-base font-bold text-purple-700">{formatCurrency(employerPension + reservationBreakdown.employeePension)}</div>
                        <div className="text-[10px] text-purple-600/80 font-medium uppercase tracking-wide flex items-center gap-1">
                             <PiggyBank className="w-3 h-3" /> Pensioen inleg
                        </div>
                        <div className="text-[10px] text-purple-600/60">Voor later</div>
                    </div>
                </div>
            </div>
        </div>
      ...
      <section className="max-w-3xl mx-auto px-4 pb-16">
        <Accordion type="single" collapsible className="w-full">
            <AccordionItem value="breakdown" className="border border-slate-200 rounded-xl overflow-hidden bg-white shadow-sm">
                <AccordionTrigger className="px-6 py-4 hover:no-underline hover:bg-slate-50 transition-colors">
                    <div className="flex flex-col items-start text-left">
                        <span className="text-lg font-bold text-slate-900">Volledige opbouw van jouw tarief</span>
                        <span className="text-sm text-slate-500 font-normal">Zo wordt elke stap van tarief naar netto loon berekend</span>
                    </div>
                </AccordionTrigger>
                <AccordionContent className="px-0 pb-0">
                    <div className="divide-y divide-slate-100">
                        ...
                        <div className="p-6 bg-slate-50/50 space-y-4">
                            <div className="flex items-center justify-between">
                                <h3 className="font-semibold text-slate-700">Kosten en marge</h3>
                                <span className="text-[10px] text-slate-500 max-w-[50%] text-right leading-tight">
                            ...
                            />
                            <div className="flex justify-between items-center pt-2 border-t border-slate-200 text-slate-500 text-sm font-medium">
                        ...
                        <div className="p-6 bg-white space-y-4 relative overflow-hidden">
                             ...
                             <div className="flex items-center justify-between">
                                <h3 className="font-semibold text-slate-700">Van kandidaat tarief naar bruto loon</h3>
                            ...
                            <div className="flex justify-between items-center text-sm font-medium text-slate-900 pb-2 border-b border-slate-100">
                                 <span>Kandidatentarief (Beschikbaar)</span>
                                 <span>{formatCurrency(candidateTotal)} <span className="text-xs font-normal text-slate-500">({formatHourly(candidateTotal)})</span></span>
                            </div>
                            <BreakdownRow 
                            ...
                            <div className="mt-6 p-3 bg-slate-50 rounded-lg flex justify-between items-center text-sm">
                                <span className="font-medium text-slate-600">Bruto Maandsalaris</span>
                                <ArrowRight className="w-4 h-4 text-slate-400" />
                                <span className="font-bold text-slate-900">{formatCurrency(grossTotal)} <span className="text-xs font-normal text-slate-500">({formatHourly(grossTotal)})</span></span>
                            </div>
                        </div>
                        ...
                        <div className="p-6 bg-white space-y-4 relative overflow-hidden">
                             ...
                             <div className="flex items-center justify-between">
                                <h3 className="font-semibold text-slate-700">Van bruto naar netto</h3>
                                <span className="text-[10px] text-slate-600/80 max-w-[50%] text-right leading-tight">
                                    Net als op je loonstrook: wat er van je bruto salaris afgaat
                                </span>
                            </div>
                            
                            {/* Starting point: Gross Salary */}
                            <div className="p-3 bg-slate-50 rounded-lg flex justify-between items-center text-sm border border-slate-200">
                                <span className="font-medium text-slate-600">Bruto Maandsalaris</span>
                                <span className="font-bold text-slate-900">{formatCurrency(grossTotal)} <span className="text-xs font-normal text-slate-500">({formatHourly(grossTotal)})</span></span>
                            </div>
                            ...
                            <div className="pl-4 space-y-4 border-l-2 border-slate-200">
                                ...
                                {/* Total Deductions */}
                                <div className="flex justify-between items-center pt-2 border-t border-slate-300 text-slate-700 text-sm font-semibold">
                            ...
                            <div className="bg-blue-700/50 rounded-lg p-4 border border-blue-400/30">
                                ...
                                <div className="mt-4 pt-4 border-t border-blue-400/30">
                                    ...
                                    <div className="bg-purple-900/30 rounded p-3 border border-purple-400/30">
                                        ...
                                        <div className="text-[10px] text-purple-200 mt-1">wordt elke maand voor je opgebouwd</div>
                                    </div>
                                </div>
      ...
      <section className="max-w-5xl mx-auto px-4">
        <div className="text-center mb-10">
            <h2 className="text-3xl font-bold text-slate-900">Wat is het verschil met ZZP?</h2>
        </div>
        ...
        <div className="grid md:grid-cols-2 gap-8">
            ...
            <div className="bg-white border border-slate-200 rounded-2xl p-8 shadow-sm relative overflow-hidden">
                <div className="absolute top-0 left-0 w-full h-1 bg-slate-400"></div>
                 <h3 className="text-xl font-bold text-slate-900 mb-6 flex items-center gap-2">
                    <Briefcase className="w-5 h-5 text-slate-500" /> ZZP
                </h3>
                 <ul className="space-y-4">
...
function BreakdownRow({ label, value, highlight = false, monthlyHours, tooltip }: { label: string, value: number, highlight?: boolean, monthlyHours: number, tooltip?: string | React.ReactNode }) {
    ...
    return (
        <div className="flex justify-between items-center text-sm gap-2">
            <span className={clsx("text-slate-600 flex items-center gap-1.5", highlight && "text-teal-700 font-medium")}>
                {label}
                {tooltip && <InfoTooltip content={tooltip} side="right" />}
            </span>
            <div className="text-right">
                <span className={clsx("font-medium block", highlight ? "text-teal-700" : "text-slate-900")}>
